...
@section('tituloPagina', 'Registrar Actividad')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <h1 class="font-display font-black text-2xl text-slate-800">Nueva Actividad Laboral</h1>
            <p class="text-xs text-slate-500">Asigna labores a operarios en campo con cálculo dinámico de subtotales</p>
        </div>
    </div>
    ...
    <div class="glass-card overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 bg-gradient-to-r from-slate-50/90 to-emerald-50/30 flex items-center gap-3">
            ...
            <div>
                <h3 class="font-display font-bold text-base text-slate-800">Detalles de la Labor</h3>
                <span class="text-xs text-slate-500">Completa los datos del trabajador, lote y tarifa vigente</span>
            </div>
        </div>

        <form method="POST" action="{{ route('actividades.store') }}" id="formActividad" class="p-6 sm:p-8 space-y-6">
            @csrf
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <select name="trabajador_id" required
                                class="w-full pl-10 pr-4 py-3 rounded-xl border @error('trabajador_id') border-rose-400 bg-rose-50/30 @else border-slate-200 bg-white @enderror text-slate-800 text-sm font-medium focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-sm">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <select name="lote_id" required
                                class="w-full pl-10 pr-4 py-3 rounded-xl border @error('lote_id') border-rose-400 bg-rose-50/30 @else border-slate-200 bg-white @enderror text-slate-800 text-sm font-medium focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-sm">
            ...
            <div class="space-y-1.5">
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600">
                    Tipo de Actividad & Tarifa Vigente <span class="text-rose-500">*</span>
                </label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                    ...
                    <select name="valor_actividad_id" id="tarifaSelect" required
                            class="w-full pl-10 pr-4 py-3 rounded-xl border @error('valor_actividad_id') border-rose-400 bg-rose-50/30 @else border-slate-200 bg-white @enderror text-slate-800 text-sm font-medium focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-sm">
                        <option value="" data-valor="0" data-unidad="">-- Seleccione la tarifa correspondiente --</option>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="date" name="fecha" id="fechaInput" value="{{ old('fecha', date('Y-m-d')) }}" max="{{ date('Y-m-d') }}" required
                               class="w-full pl-10 pr-4 py-3 rounded-xl border @error('fecha') border-rose-400 bg-rose-50/30 @else border-slate-200 bg-white @enderror text-slate-800 text-sm font-medium focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-sm">
                    </div>
                    @error('fecha')
                        <p class="text-xs text-rose-500 font-semibold flex items-center gap-1 mt-1">
                            <i class="fas fa-circle-exclamation"></i> {{ $message }}
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600">
                        Cantidad <span id="unidadLabel" class="text-emerald-600 lowercase font-normal"></span> <span class="text-rose-500">*</span>
                    </label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                            <i class="fas fa-calculator"></i>
                        </div>
                        <input type="number" name="cantidad" id="cantidadInput" value="{{ old('cantidad', 1) }}" min="1" required
                               class="w-full pl-10 pr-4 py-3 rounded-xl border @error('cantidad') border-rose-400 bg-rose-50/30 @else border-slate-200 bg-white @enderror text-slate-800 text-sm font-medium font-mono focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-sm">
                    </div>
                    @error('cantidad')
                        <p class="text-xs text-rose-500 font-semibold flex items-center gap-1 mt-1">
                            <i class="fas fa-circle-exclamation"></i> {{ $message }}
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="numero_pasada" id="pasadaInput" value="{{ old('numero_pasada', 1) }}" min="1" required
                               class="w-full pl-10 pr-4 py-3 rounded-xl border @error('numero_pasada') border-rose-400 bg-rose-50/30 @else border-slate-200 bg-white @enderror text-slate-800 text-sm font-medium font-mono focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-sm">
                    </div>
                    @error('numero_pasada')
                        <p class="text-xs text-rose-500 font-semibold flex items-center gap-1 mt-1">
                            <i class="fas fa-circle-exclamation"></i> {{ $message }}
            ...
            <div class="p-5 rounded-2xl bg-gradient-to-r from-emerald-500/10 via-emerald-500/5 to-transparent border border-emerald-400/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div id="subtotalFormula" class="text-xs text-slate-500 font-mono mt-0.5">Selecciona tarifa, cantidad y pasadas</div>
                    </div>
                </div>
                <div class="text-right sm:text-right w-full sm:w-auto">
                    <div id="subtotalValor" class="font-display font-black text-2xl sm:text-3xl text-emerald-700 font-mono tracking-tight">$0</div>
                </div>
            </div>
